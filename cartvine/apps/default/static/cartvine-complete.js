 ...
 * MIT Licensed
 */
Davis=function(a){var b=new Davis.App;a&&a.call(b);Davis.$(function(){b.start()});return b};if(window.jQuery){Davis.$=jQuery}else{Davis.$=null}Davis.supported=function(){return(typeof window.history.pushState=="function")}
/*!
 ...
 * @returns {Function}
 */
;Davis.noop=function(){};Davis.extend=function(a){a(Davis)}
/*!
 * the version
 */
;Davis.version="0.9.1";
...
/*!
   ...
   * @returns {Array}
   */
if(Array.prototype.every){var e=function(g,f){return g.every(f,arguments[2])}}else{var e=function(l,k){if(l===void 0||l===null){throw new TypeError()}var j=Object(l);var f=j.length>>>0;if(typeof k!=="function"){throw new TypeError()}var h=arguments[2];for(var g=0;g<f;g++){if(g in j&&!k.call(h,j[g],g,j)){return false}}return true}}
/*!
   ...
   * @returns {Array}
   */
if(Array.prototype.forEach){var d=function(g,f){return g.forEach(f,arguments[2])}}else{var d=function(l,k){if(l===void 0||l===null){throw new TypeError()}var j=Object(l);var f=j.length>>>0;if(typeof k!=="function"){throw new TypeError()}var h=arguments[2];for(var g=0;g<f;g++){if(g in j){k.call(h,j[g],g,j)}}}}
/*!
   ...
   * @returns {Array}
   */
if(Array.prototype.filter){var b=function(g,f){return g.filter(f,arguments[2])}}else{var b=function(n,l){if(n===void 0||n===null){throw new TypeError()}var k=Object(n);var f=k.length>>>0;if(typeof l!=="function"){throw new TypeError()}var j=[];var h=arguments[2];for(var g=0;g<f;g++){if(g in k){var m=k[g];if(l.call(h,m,g,k)){j.push(m)}}}return j}}
/*!
   ...
   * @returns {Array}
   */
if(Array.prototype.map){var c=function(g,f){return g.map(f,arguments[2])}}else{var c=function(m,l){if(m===void 0||m===null){throw new TypeError()}var k=Object(m);var f=k.length>>>0;if(typeof l!=="function"){throw new TypeError()}var j=new Array(f);var h=arguments[2];for(var g=0;g<f;g++){if(g in k){j[g]=l.call(h,k[g],g,k)}}return j}}
/*!
   ...
   * @returns {Array}
   */
var a=function(f,g){var g=g||0;return Array.prototype.slice.call(f,g)}
/*!
   * Exposing the public interface to the Utils module
   * @private
   */
;return{every:e,forEach:d,filter:b,toArray:a,map:c}})()
/*!
 * Davis - listener
 * Copyright (C) 2011 Oliver Nightingale
 * MIT Licensed
...
/*!
   ...
   * @private
   */
var a={A:function(g){return g.host!==window.location.host||g.protocol!==window.location.protocol},FORM:function(h){var g=document.createElement("a");g.href=h.action;return this.A(g)}}
/*!
   ...
   * @private
   */
;var b=function(g){if(!a[g.nodeName.toUpperCase()]){return true}return a[g.nodeName.toUpperCase()](g)}
/*!
   ...
   * @private
   */
;var c=function(g){return function(i){if(b(this)){return true}var h=new Davis.Request(g.call(Davis.$(this)));Davis.location.assign(h);i.stopPropagation();i.preventDefault();return false}};
/*!
   * A handler specialized for click events.  Gets the request details from a link elem
   * @private
   */
var f=c(function(){var g=this;return{method:"get",fullPath:this.attr("href"),title:this.attr("title"),delegateToServer:function(){window.location.pathname=g.attr("href")}}});
/*!
   * Decodes the url, including + characters.
   * @private
   */
var e=function(g){return decodeURIComponent(g.replace(/\+/g,"%20"))};
/*!
   * A handler specialized for submit events.  Gets the request details from a form elem
   * @private
   */
var d=c(function(){var g=this;return{method:this.attr("method"),fullPath:e(this.serialize()?[this.attr("action"),this.serialize()].join("?"):this.attr("action")),title:this.attr("title"),delegateToServer:function(){g.submit()}}});this.listen=function(){Davis.$(document).delegate(this.settings.formSelector,"submit",d);Davis.$(document).delegate(this.settings.linkSelector,"click",f)};this.unlisten=function(){Davis.$(document).undelegate(this.settings.linkSelector,"click",f);Davis.$(document).undelegate(this.settings.formSelector,"submit",d)}}
/*!
 * Davis - event
 * Copyright (C) 2011 Oliver Nightingale
 * MIT Licensed
...
/*!
   * callback storage
   */
var a={};this.bind=function(c,b){(a[c]=a[c]||[]).push(b);return this};this.trigger=function(f){var d=Davis.utils.toArray(arguments,1),c=a[f];if(!c){return this}for(var e=0,b=c.length;e<b;++e){c[e].apply(this,d)}return this}}
/*!
 * Davis - logger
 * Copyright (C) 2011 Oliver Nightingale
 * MIT Licensed
...
/*!
   ...
   * @private
   */
function d(){return"["+Date()+"]"}
/*!
   * Pushing the timestamp onto the front of the arguments to log
   * @private
   */
function c(h){var g=Davis.utils.toArray(h);g.unshift(d());return g.join(" ")}var b=function(g){return function(){if(window.console){console[g](c(arguments))}}};var a=b("error");var e=b("info");var f=b("warn")
/*!
   * Exposes the public methods of the module
   * @private
   */
;this.logger={error:a,info:e,warn:f}}
/*!
 * Davis - Route
 * Copyright (C) 2011 Oliver Nightingale
 * MIT Licensed
 */
;Davis.Route=(function(){var b=/:([\w\d]+)/g;var f="([^/]+)";var d=/\*([\w\d]+)/g;var a="(.*)";var e=/[:|\*]([\w\d]+)/g;var c=function(l,k,i){var j=function(){if(!(k instanceof RegExp)){var m=k.replace(b,f).replace(d,a);k.lastIndex=0;return new RegExp("^"+m+"$","gi")}else{return k}};var h=function(){if(!(l instanceof RegExp)){return new RegExp("^"+l+"$","i")}else{return l}};var g=function(){var n=[],m;while((m=e.exec(k))){n.push(m[1])}return n};this.paramNames=g();this.path=j();this.method=h();if(typeof i==="function"){this.handlers=[i]}else{this.handlers=i}};c.prototype.match=function(h,g){this.reset();return(this.method.test(h))&&(this.path.test(g))};c.prototype.reset=function(){this.method.lastIndex=0;this.path.lastIndex=0};c.prototype.run=function(j){this.reset();var k=this.path.exec(j.path);if(k){k.shift();for(var h=0;h<k.length;h++){j.params[this.paramNames[h]]=k[h]}}var g=Davis.utils.map(this.handlers,function(m,l){return function(i){return m.call(i,i,g[l+1])}});return g[0](j)};c.prototype.toString=function(){return[this.method,this.path].join(" ")}
/*!
 ...
 * MIT Licensed
 */
;Davis.router=function(){this.route=function(f,e){var d=function(j){var h=Davis.utils.toArray(arguments,1),i=a.join(""),g=new Davis.Route(f,i+j,h);c.push(g);return g};return(arguments.length==1)?d:d.apply(this,Davis.utils.toArray(arguments,1))};this.get=this.route("get");this.post=this.route("post");this.put=this.route("put");this.del=this.route("delete");this.state=this.route("state");this.scope=function(e,d){a.push(e);if(arguments.length==1){return}d.call(this,this);a.pop()};this.trans=function(g,f){if(f){var d=[g,decodeURIComponent(Davis.$.param(f))].join("?")}else{var d=g}var e=new Davis.Request({method:"state",fullPath:d,title:""});Davis.location.assign(e)}
/*!
   * Generating convinience methods for creating filters using Davis.Routes and methods to
   * lookup filters.
   */
;this.filter=function(d){return function(){var h=/.+/;if(arguments.length==1){var g=/.+/;var f=arguments[0]}else{if(arguments.length==2){var g=arguments[0];var f=arguments[1]}}var e=new Davis.Route(h,g,f);b[d].push(e);return e}};this.lookupFilter=function(d){return function(f,e){return Davis.utils.filter(b[d],function(g){return g.match(f,e)})}};this.before=this.filter("before");this.after=this.filter("after");this.lookupBeforeFilter=this.lookupFilter("before");this.lookupAfterFilter=this.lookupFilter("after")
/*!
   * collections of routes and filters
   * @private
   */
;var c=[];var b={before:[],after:[]};var a=[];this.lookupRoute=function(e,d){return Davis.utils.filter(c,function(f){return f.match(e,d)})[0]}}
/*!
 * Davis - history
 * Copyright (C) 2011 Oliver Nightingale
 * MIT Licensed
...
/*!
   ...
   * @private
   */
;function d(){if("state" in window.history){return true}else{return c}}
/*!
...
/*!
   ...
   * @private
   */
function b(m){window.addEventListener("popstate",m,true)}
/*!
   ...
   * @private
   */
function h(m){return function(n){if(n.state&&n.state._davis){m(new Davis.Request(n.state._davis))}else{if(d()){m(Davis.Request.forPageLoad())}c=true}}}
/*!
   ...
   * @private
   */
function g(m){return{_davis:m}}function f(m){k(m);b(h(m))}
/*!
   ...
   * @private
   */
function j(m){return function(o,n){history[m](g(o.toJSON()),o.title,o.location());if(n&&n.silent){return}Davis.utils.forEach(i,function(p){p(o)})}}var l=j("pushState");var a=j("replaceState");function e(){return window.location.pathname+(window.location.search?window.location.search:"")}
/*!
   * Exposing the public methods of this module
   * @private
   */
return{onChange:f,current:e,assign:l,replace:a}})()
/*!
 * Davis - location
 * Copyright (C) 2011 Oliver Nightingale
 * MIT Licensed
...
/*!
   ...
   */
var d=Davis.history;function e(h){d=h}function f(){return d.current()}
/*!
   ...
   * @private
   */
function g(h){return function(i){if(typeof i=="string"){i=new Davis.Request(i)}d[h](i)}}var b=g("assign");var c=g("replace");function a(h){d.onChange(h)}
/*!
   * Exposing the public methods of this module
   * @private
   */
return{setLocationDelegate:e,current:f,assign:b,replace:c,onChange:a}})()
/*!
 * Davis - Request
 * Copyright (C) 2011 Oliver Nightingale
 * MIT Licensed
 */
;Davis.Request=(function(){var a=function(d,e){if(typeof d=="object"){e=d;d=e.fullPath;delete e.fullPath}var c=Davis.$.extend({},{title:"",fullPath:d,method:"get"},e);var b=this;this.raw=c;this.params={};this.title=c.title;this.queryString=c.fullPath.split("?")[1];this._staleCallback=function(){};if(this.queryString){Davis.utils.forEach(this.queryString.split("&"),function(i){var k=i.split("=")[0],l=i.split("=")[1],j=/^(\w+)\[(\w+)?\](\[\])?/,m;if(m=j.exec(k)){var f=m[1];var k=m[2];var g=!!m[3];var h=b.params[f]||{};if(g){h[k]=h[k]||[];h[k].push(decodeURIComponent(l));b.params[f]=h}else{if(!k&&!g){h=b.params[f]||[];h.push(decodeURIComponent(l));b.params[f]=h}else{h[k]=decodeURIComponent(l);b.params[f]=h}}}else{b.params[k]=decodeURIComponent(l)}})}c.fullPath=c.fullPath.replace(/^https?:\/\/.+?\//,"/");this.method=(this.params._method||c.method).toLowerCase();this.path=c.fullPath.replace(/\?.+$/,"").replace(/^https?:\/\/[^\/]+/,"");this.fullPath=c.fullPath;this.delegateToServer=c.delegateToServer||Davis.noop;this.isForPageLoad=c.forPageLoad||false;if(a.prev){a.prev.makeStale(this)}a.prev=this};a.prototype.redirect=function(b){Davis.location.replace(new a({method:"get",fullPath:b,title:this.title}))};a.prototype.whenStale=function(b){this._staleCallback=b};a.prototype.makeStale=function(b){this._staleCallback.call(b,b)};a.prototype.location=function(){return(this.method==="get")?this.fullPath:""};a.prototype.toString=function(){return[this.method.toUpperCase(),this.path].join(" ")};a.prototype.toJSON=function(){return{title:this.raw.title,fullPath:this.raw.fullPath,method:this.raw.method}};a.forPageLoad=function(){return new this({method:"get",fullPath:Davis.location.current(),title:document.title,forPageLoad:true})}
/*!
   * Stores the last request
   * @private
   */
;a.prev=null;return a})()
/*!
 * Davis - App
 * Copyright (C) 2011 Oliver Nightingale
 * MIT Licensed
 */
;Davis.App=(function(){function a(){this.running=false;this.boundToInternalEvents=false;this.use(Davis.listener);this.use(Davis.event);this.use(Davis.router);this.use(Davis.logger)}a.prototype.configure=function(b){b.call(this.settings,this.settings)};a.prototype.use=function(b){b.apply(this,Davis.utils.toArray(arguments,1))};a.prototype.helpers=function(b){for(property in b){if(b.hasOwnProperty(property)){Davis.Request.prototype[property]=b[property]}}};a.prototype.settings={linkSelector:"a",formSelector:"form",throwErrors:true,handleRouteNotFound:false,generateRequestOnPageLoad:false};a.prototype.start=function(){var b=this;if(this.running){return}if(!Davis.supported()){this.trigger("unsupported");return}var c=function(g){return function(i){var h=i.run(g,g);return(typeof h==="undefined"||h)}};var d=function(g){return Davis.utils.every(b.lookupBeforeFilter(g.method,g.path),c(g))};var f=function(i){if(d(i)){b.trigger("lookupRoute",i);var g=b.lookupRoute(i.method,i.path);if(g){b.trigger("runRoute",i,g);try{g.run(i);b.trigger("routeComplete",i,g)}catch(h){b.trigger("routeError",i,g,h)}Davis.utils.every(b.lookupAfterFilter(i.method,i.path),c(i))}else{b.trigger("routeNotFound",i)}}else{b.trigger("requestHalted",i)}};var e=function(){b.bind("runRoute",function(g){b.logger.info("runRoute: "+g.toString())}).bind("routeNotFound",function(g){if(!b.settings.handleRouteNotFound&&!g.isForPageLoad){b.stop();g.delegateToServer()}b.logger.warn("routeNotFound: "+g.toString())}).bind("start",function(){b.logger.info("application started")}).bind("stop",function(){b.logger.info("application stopped")}).bind("routeError",function(i,g,h){if(b.settings.throwErrors){throw (h)}b.logger.error(h.message,h.stack)});Davis.location.onChange(function(g){f(g)});b.boundToInternalEvents=true};if(!this.boundToInternalEvents){e()}this.listen();this.trigger("start");this.running=true;if(this.settings.generateRequestOnPageLoad){f(Davis.Request.forPageLoad())}};a.prototype.stop=function(){this.unlisten();this.trigger("stop");this.running=false};return a})();